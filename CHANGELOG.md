...
<!-- Personal note: Ran into an issue where the OAuth2 flow would open the browser but I was already logged into the wrong X account in that browser session. Easy to miss since xurl doesn't warn you which account it expects. Workaround: open an incognito window manually before running `xurl auth login`, then paste the URL there. TODO: check if upstream plans to add a --no-browser flag that just prints the URL instead of auto-opening it — that would be much cleaner for multi-account setups. -->
