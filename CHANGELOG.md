...
<!-- Personal note: Noticed that token expiry errors can look identical to network errors in the CLI output, making it hard to tell whether a failed request is a connectivity issue or just a stale token. As a quick self-reminder: if a request fails with a generic error and the last auth was more than ~2 hours ago, try `xurl auth refresh` first before debugging anything else. -->
