...
<!-- Personal note: Another gotcha I ran into — if you have multiple apps configured in ~/.xurl and you switch between them frequently, it's easy to accidentally trigger a token refresh for the wrong app. I added `xurl auth status` to my shell prompt snippet (PS1) so I can always see which app/user is active. Probably overkill but it saved me some confusion during testing. -->
